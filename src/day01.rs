...
impl  Sonar{
    fn new(data: &str) -> Sonar{
        let measurements: Vec<u32> = data.lines().map(|line| line.parse::<u32>().unwrap()).collect();
        Sonar {measurements: measurements}
    }
    ...
    fn count_increments(&self, window_size: usize) -> usize {
        ...
        slice_sums.windows(2).map(|w| w[1] > w[0]).filter(|&n| n).count()
    }
}
